...
const SESSION_COOKIE_NAME = "health_worker_session";
...
export type AuthenticatedHealthWorker = {
  id: string;
  email: string;
  name: string;
  role: AdminRole;
...
function getSessionSecret() {
  ...
  if (process.env.NODE_ENV !== "production") {
    return "development-only-health-worker-session-secret";
  }
...
export async function createHealthWorkerSession(adminId: string) {
...
export async function clearHealthWorkerSession() {
  const cookieStore = await cookies();
  cookieStore.delete(SESSION_COOKIE_NAME);
}

export async function getCurrentHealthWorkerFromSession(): Promise<AuthenticatedHealthWorker | null> {
  const cookieStore = await cookies();
  ...
  if (!admin || admin.role !== AdminRole.HEALTH_WORKER) {
    return null;
  }
